...
class Tx8:
    ...
    def execute(self, i_params, o_params, w_params, cp, cm_size, cm_type, cm_hops):
        ...
        if len_cp != 1:#融合算子的计算时间
            ...
            for i in range(len_cp):
                ...
                if cpi[1] == 0:
                    w_cm_flag = 0
                    n = i_params[1] / self.config["4X4 tile"]#i_params[1] / self.config["4X4 tile"]为n1
                else:
                    n = (i_params[1] / self.config["4X4 tile"]) * (w_params[1] / self.config["4X4 tile"])#w_params[1] / self.config["4X4 tile"]为n2
                    w_cm_flag = self.config["4X4 tile"] - 1                
                
                cp_latency_per = self.Computation_latency(cp[i])#每一次的计算时间
                ...
                #print(cp_latency_per,cp_latency_cycle,n)
        else:#单个算子的计算时间统计
            if i_params[1] == o_params[1] == w_params[1]:
                w_cm_flag = 0
                n = i_params[1] / self.config["4X4 tile"]
            else:
            ...
            cp_latency_per = self.Computation_latency(cp[0])#每一次的计算时间
            cp_latency_cycle = (w_cm_flag + 1) * cp_latency_per#每一轮的计算时间
            total_cp_latency = n * cp_latency_cycle#总计算时间
            
        
        #n = math.ceil(i_params[1] / self.config["4X4 tile"]) * math.ceil(w_params[1] / self.config["4X4 tile"])#计算出流水轮次n
        ...
        cm_latency_per_noc, cm_latency_per_overlap = self.Communication_latency(cm_size, cm_type, cm_hops)#每一次noc通信时间和overlap时间
        ...
        if len_cp == 1:
            dram_read_cycle = self.DRAM_read(i_params, w_params, local_last, cp, len_cp)#每一轮DRAM读取数据的时间
            total_dram_read = n * dram_read_cycle#总DRAM读取时间
            dram_store_per = self.DRAM_store(i_params, o_params, w_params, local_next)#每一次DRAM存储数据的时间
            ...
            assert i_params[1] * w_params[1] == o_params[1] or i_params[1] == o_params[1]#条件不满足，程序退出
            
            if i_params[1] * w_params[1] == o_params[1]:#GEMM
                # 计算不同情况下的总延迟
                if max_variable ==  "cp_latency_cycle":
                    latency = dram_read_cycle + dram_store_per + total_cp_latency
                elif max_variable ==  "cm_latency_cycle":
                    latency = dram_read_cycle + dram_store_per + total_cm_latency
                elif max_variable ==  "dram_read":
                    latency = total_dram_read + dram_store_per + cp_latency_per
                else:
                    latency = total_dram_store + dram_read_cycle + cp_latency_per
                    #print( total_dram_store, dram_read , cp_latency)
                    #print('latency',latency)
            else:#flashattention
                # 计算不同情况下的总延迟
                if max_variable ==  "cp_latency_cycle":
                    latency = dram_read_cycle + dram_store_cycle + total_cp_latency
                elif max_variable ==  "cm_latency_cycle":
                    latency = dram_read_cycle + dram_store_cycle + total_cm_latency
                elif max_variable ==  "dram_read":
                    latency = total_dram_read + dram_store_cycle + cp_latency_per
                else:
                    latency = total_dram_store + dram_read_cycle + cp_latency_per
            Utilization = total_cp_latency / latency
            
        else:#len_cp != 1，即算子融合的情况
            total_dram_read = 0
            ...
            if i_params[1] * w_params[1] == o_params[1]:#GEMM        
                latency = dram_read_list[0] + total_cp_latency + dram_store_per
            else:
                latency = dram_read_list[0] + total_cp_latency + dram_store_cycle       
    ...
    # 该函数用于验证SRAM是否够用。如果切分数量小于等于16，则不需要遵循SRAM的2-3-2分配策略（2输入，3权重，2输出） 
        ...
        if i_params[1] == o_flag == w_params[1]:#Vector算子情况，如RMSNorm
            if i_params[1] <= self.config["4X4 tile"]:
                if i_params[0] + w_params[0] <= 3:
                    verification_result = True
                else:
                    verification_result = False
            else:
                if 2 * i_params[0] + 2 * w_params[0] <= 3:
                    verification_result = True
                else:
                    verification_result = False

        else:
            if i_params[1] and o_flag <= self.config["4X4 tile"] and w_params[1] <= self.config["4X4 tile"] and i_params[1] == o_flag:
                if i_params[0] + w_params[0] <= 3:
                    verification_result = True
                else:
                    verification_result = False
        
            elif i_params[1] and o_flag <= self.config["4X4 tile"] and w_params[1] <= self.config["4X4 tile"] and i_params[1] != o_flag:
                if i_params[0] + o_params[0] + w_params[0] <= 3:
                    verification_result = True
                else:
                    verification_result = False
                
            elif i_params[1] <= self.config["4X4 tile"] and o_flag <= self.config["4X4 tile"] and w_params[1] > self.config["4X4 tile"]:
    ...
    def Computation_latency(self, cp_n):
        if cp_n[1] == 0:
            cp_latency = cp_n[0] / self.config["Vector"]#cp_type=0为Vector计算
        else:
            cp_latency = cp_n[0] / self.config["Gemm"]#cp_type=1为GEMM计算
        return cp_latency
