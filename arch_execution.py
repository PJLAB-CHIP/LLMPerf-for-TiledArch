...
class Tx8:
    ...
    def execute(self, i_params, o_params, w_params, cp, cm_size, cm_type, cm_hops):
        ...
        if len_cp == 1:#仅有一个算子时
            ...
            #assert dram_store_cycle < cp_latency_cycle#默认存储时间能隐藏在计算时间里
            # 计算一次 noc片上流转的时间
            ...
            latency_cycle = max(time_one_noc_pipe_flow, dram_read_cycle)
            
            #print(cp_latency_cycle,dram_read_cycle,dram_store_cycle)
            ...
            #assert dram_store_cycle < cp_latency_cycle[1]#默认存储时间能隐藏在计算时间里
            latency_cycle = max(cp_latency_cycle[1], cm_latency_cycle, dram_read_cycle[1])
        ...
        if len_cp == 1:#仅有一个算子时
            ...
            if i_params[1] <= w_params[1]:
                ...
                dram_read_cycle_in = n2 * dram_read_cycle + w_params[0] * self.config["TILE_NUM"] / self.config["DRAM_BW(GB/s)"]#每一个完整内层循环的DRAM读取数据时间
            
            dram_store_cycle_in = n2 * dram_store_cycle#每一个完整内层循环的DRAM存储数据时间
            
            latency_cycle_in = n2 * latency_cycle
            print(dram_read_cycle_in,dram_store_cycle_in)
            print(latency_cycle_in)
            ...
            latency_cycle_in = n2 * latency_cycle + dram_read_cycle[1]
        ...
        if len_cp == 1:#仅有一个算子时
            ...
            if i_params[1] <= w_params[1]:
                dram_read_cycle_in_P = dram_read_cycle_in_I
            else:
                dram_read_cycle_in_P = dram_read_cycle_in_W
            
            if cp_latency_cycle < dram_read_cycle_in_P + dram_read_cycle:
                latency = dram_read_cycle + dram_read_cycle_in_P + n1 * latency_cycle_in + dram_store_per# + (n1 - 1) * (dram_read_cycle_in_P + 2 * dram_read_cycle - 2 * cp_latency_cycle)
            else:
                latency = dram_read_cycle + dram_read_cycle_in_P + n1 * latency_cycle_in + dram_store_per            
        else:
            ...
            if len_cp == 2:
                latency = dram_read_cycle_in_P + n1 * latency_cycle_in + dram_store_per
            else:
                latency = dram_read_cycle_in_P + n1 * latency_cycle_in + dram_store_cycle
    ...
    # 该函数用于验证SRAM是否够用。如果切分数量小于等于16，则不需要遵循SRAM的2-3-2分配策略（2输入，3权重，2输出） 
        ...
        if i_params[1] == o_flag == w_params[1]:#Vector算子情况，如RMSNorm
            ...
            if i_params[1] <= self.config["TILE_NUM"] and w_params[1] <= self.config["TILE_NUM"]:
                if i_params[0] + w_params[0] + o_params[0]<= self.config["SRAM(MB)"]:
                    verification_result = True
                else:
                    verification_result = False
            elif i_params[1] <= self.config["TILE_NUM"] and w_params[1] > self.config["TILE_NUM"]:
                if i_params[0] + 3 * w_params[0] + 2 * o_params[0]<= self.config["SRAM(MB)"]:
                    verification_result = True
                else:
                    verification_result = False
            elif i_params[1] > self.config["TILE_NUM"] and w_params[1] <= self.config["TILE_NUM"]:
                if 2 * i_params[0] + w_params[0] + 2 * o_params[0]<= self.config["SRAM(MB)"]:
                    verification_result = True
                else:
                    verification_result = False
            else:
                if i_params[1] <= w_params[1]:
                    ...
                    if 3 * i_params[0] + 2 * w_params[0] + 2 * o_params[0]<= self.config["SRAM(MB)"]:
                        verification_result = True
                    else:
                        verification_result = False
