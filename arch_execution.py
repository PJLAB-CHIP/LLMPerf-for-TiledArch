...
class Tx8:
            ...
            if i_params[1] <= self.config["TILE_NUM"] and w_params[1] <= self.config["TILE_NUM"]:
                ...
                if i_params[1] <= w_params[1]:
                    ...
                    if 3 * i_params[0] + 2 * w_params[0] + 2 * o_params[0] <= self.config["SRAM(MB)"]:
                        ...
                        if 3 * i_params[0] + w_params[0] + 2 * o_params[0] <= self.config["SRAM(MB)"]:
                            print("输入不复则SRAM满足要求")
                            verification_flag = 1
                            verification_result = True
                        else:
    ...
    def FlashAttention(self, i_params, o_params, w_params, cp, cm_size, cm_type, cm_hops, verification_flag):
        ...
        iter_over_input_time = (n1 - 1) * time_one_iter_in
        last_iter_time = time_one_noc_pipe_flow
        latency = initial_load_time + iter_over_weight_time + iter_over_input_time + last_iter_time + dram_store_cycle
