...
def gemm_auto_opt_mapper(op, arch, input_stationary=True, Tm_Tn=None, fusion_op1=None, fusion_op2=None, details=False):
    ...
    if input_stationary:
        ...
        dims = [1]+op['wshape']+[op['ishape'][0]*op['ishape'][1]]
        # print(dims)
...
def flashatten_mapper(model, arch, Tx_Ty=None, details=True, Head_fused=True):
    ...
    for tx in Tx:  # outer Q
        for ty in Ty:
            ...
            print('test', i_params, o_params, w_params,
                  cp, cm_size, cm_type, cm_hops)
            sram_cap_req, total_cp_latency, _, _, tot_latency, tot_utilization = arch.execute(
                i_params, o_params, w_params, cp,  cm_size, cm_type, cm_hops)
...
def vector_mapper(op, arch, splits=None, details=False):
    ...
    assert (op['name'] in ['RMSNorm', 'RMSNorm2', 'Hadamard',
            'ResAdd', 'ResAdd2',]) and (op['type'] == 'Vector')
...
def manual_mapper(model, arch, QKV_fusion=True, preset=True, details=True):
    ...
    mapping_result['Total'] = {
        "latency": tot_latency, 'utilization': tot_cp_latency/tot_latency, 'cp_latency': tot_cp_latency}
    print('{:<15}, latency(ms)={:>10.6f}, utilization(%)={:>10.6f}, compute latency(ms)={:>10.6f}'.format(
