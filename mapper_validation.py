...
def manual_mapper(model,arch,QKV_fusion=True,preset=True,details=True):
    ...
    if QKV_fusion:
        ...
        Nm_Nn=[64,64] if preset else None
        # Nm_Nn=[128, 32] if preset else None
        ...
        ops['V_proj']['ishape'][0] = B
        ops['V_proj']['oshape'][0] = B
        # tile_m = 64
        # tile_n = 64
        ...
        # tile_m = 32
        # tile_n = 128
        Nm_Nn=[int(B*M/tile_m), int(N/tile_n)] 
        input_stationary = False
        mapping_result['RMSNorm&Q_proj']=gemm_auto_opt_mapper(ops['Q_proj'],arch,input_stationary = input_stationary, Nm_Nn=Nm_Nn,fusion_op1=ops['RMSNorm'],details=details)
        mapping_result['K_proj']=gemm_auto_opt_mapper(ops['K_proj'],arch,input_stationary = input_stationary,Nm_Nn=Nm_Nn,details=details)
        mapping_result['V_proj']=gemm_auto_opt_mapper(ops['V_proj'],arch,input_stationary = input_stationary,Nm_Nn=Nm_Nn,details=details)
        del ops['RMSNorm']
        del ops['Q_proj']
...
if __name__ == "__main__":
    ...
    mapping_result=manual_mapper(llama7b,hardware,QKV_fusion=False, preset=True,details=False)
