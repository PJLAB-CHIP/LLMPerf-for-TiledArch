...
class Tx8:
    ...
    # 该函数用于验证SRAM是否够用。如果切分数量小于等于16，则不需要遵循SRAM的2-3-2分配策略（2输入，3权重，2输出）
        if i_params[1] and o_params[1] <= 16 and w_params[1] <= 16:
    ...
    def Computation_latency(self, cp_size, cp_type):
        if cp_type == 0:
            cp_latency = cp_size / vector#cp_type=0为Vector计算
        else:
            cp_latency = cp_size / gemm#cp_type=1为GEMM计算
        return cp_latency


    def Communication_latency(self, cm_size, cm_type, cm_hops):
        cm_latency = cm_size / noc#通信延迟，单位为ms
        overlap = Hops * cm_hops#overlap用时，单位为us
        return cm_latency, overlap
    
    def DRAM_read(self, i_params, dram_size, dram_type, local_last):
        if local_last == 0:#不复用
            read_latency = (dram_size * 16) / dram
        else:#复用
            read_latency = (dram_size - i_params[0]) * 16 / dram
        return read_latency
    
    def DRAM_store(self, o_params, dram_size, dram_type, local_next):
        if local_next == 0:#不复用
            store_latency = (o_params[0] * 16) / dram
        else:#复用
            store_latency = 0
...
# # 验证verfication函数是否正确
# A = arch.verification([2,16], [0.25,128],[0.015625,2048,0])
# print(A)
...
verification_result, total_cp_latency, total_cm_latency, total_DRAM, latency, Utilization = arch.execute([2,16], [0.25,128],[0.015625,2048,15], 0.0625, 1, 2.25, 0, 0.25, 0, 1, 1, 5, 8)
